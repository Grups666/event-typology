...
(async()=>{
  ...
  try {
    ...
    const result=await page.evaluate(()=>{
      ...
      const pixels=ctx.getImageData(100,100,7,1).data;
      return {radii,alpha:Array.from({length:7},(_,i)=>pixels[i*4+3]),center:Array.from(pixels.slice(0,3)),outer:Array.from(pixels.slice(24,27))};
    });
    assert.deepEqual(result.radii,[2.5,3.8,5.1,7.7,8.5]);
